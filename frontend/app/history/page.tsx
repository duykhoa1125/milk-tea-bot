...
export default function HistoryPage() {
  ...
  const stats = [
    ...
    {
      label: "Revenue",
      value: `${orders.reduce((acc, o) => acc + (o.totalPrice || 0), 0).toLocaleString()}đ`,
    ...
    },
    {
      ...
      value: orders.length
        ? `${Math.round((orders.filter((o) => o.status === "DONE").length / orders.length) * 100)}%`
        : "0%",
      icon: TrendingUp,
      color: "bg-orange-500",
    },
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-secondary mt-2">
            Track and manage your past order records.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {stats.map((stat, i) => (
          <div key={i} className="modern-card p-6 flex items-center gap-6">
            <div
              className={`w-14 h-14 ${stat.color} rounded-2xl flex items-center justify-center text-white shadow-lg`}
            >
              <stat.icon size={28} />
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            {loading ? (
              Array(5)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="modern-card p-6 h-32 animate-pulse bg-gray-100"
                  />
                ))
            ) : orders.length === 0 ? (
              ...
              orders.map((order) => (
                <div
                  key={order.id}
                  className="modern-card group p-6 flex flex-col md:flex-row md:items-center justify-between gap-6"
                >
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-2">
                        {order.items.map((item: any, idx: number) => (
                          <div
                            key={idx}
                            className="bg-gray-100 text-[10px] px-2 py-1 rounded-md font-bold text-secondary space-y-0.5 max-w-55"
                          >
                            <div className="truncate">
                              {item.quantity}x {item.product.name}
                            </div>
                            {item.note && (
                              <div className="font-medium italic text-secondary/80 truncate">
                                Note: {item.note}
                              </div>
                            )}
                          </div>
                      ...
                      {order.note && (
                        <div className="mt-3 inline-flex items-start gap-2 rounded-xl border border-primary/15 bg-primary/5 px-3 py-2 text-xs italic text-primary/80 max-w-full">
                          <span className="not-italic font-black uppercase tracking-widest text-[10px] text-primary/60 shrink-0">
                            Order note
                          </span>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-8">
                    <div className="text-right">
                      <p className="text-xs font-bold text-secondary uppercase tracking-tighter mb-1">
                        Total Amount
                      </p>
        ...
        <div className="lg:col-span-4 space-y-8">
          {/* Top Selling Block */}
          <div className="modern-card p-6 bg-linear-to-br from-primary to-orange-600 text-white shadow-xl shadow-primary/20">
            <h3 className="text-xl font-black mb-1">Weekly Summary</h3>
            <p className="text-white/70 text-sm mb-6">
              Your kitchen performance this week.
            </p>
          ...
          <div className="modern-card p-6 overflow-hidden relative min-h-75 flex flex-col justify-end">
            ...
            <div className="relative z-10">
              <span className="bg-emerald-500 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest mb-3 inline-block">
                Flash Insight
              </span>
              <h3 className="text-2xl font-black text-foreground leading-tight mb-2">
                Optimize your <span className="text-primary">Peak Hours</span>
              </h3>
              <p className="text-secondary text-sm mb-6">
                Most orders happen between 2PM and 4PM. Consider adding more
                staff during this window.
              </p>
