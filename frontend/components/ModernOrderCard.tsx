      ...
      <div className="flex-1">
        ...
        <ul className="space-y-3">
          {order.items.map((item: any) => (
            <li
              key={item.id}
              className="space-y-2 bg-gray-50 p-3 rounded-xl border border-transparent hover:border-primary/20 transition-all"
            >
              <div className="flex justify-between items-center gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="w-6 h-6 bg-white border border-border rounded-lg flex items-center justify-center text-[10px] font-black text-primary shrink-0">
                    {item.quantity}
                  </span>
                ...
                <span className="text-[10px] font-black text-secondary uppercase bg-white px-2 py-0.5 rounded-md border border-border shrink-0">
                  {item.size}
                </span>
              </div>

              {item.note && (
                <div className="ml-9 text-xs italic text-primary/80 flex items-start gap-2">
                  <span className="mt-0.5 text-primary/60">Note</span>
        ...
        {order.note && (
          <div className="mt-4 p-3 bg-primary/5 rounded-xl border-l-4 border-primary text-xs italic text-primary/80 font-medium">
            <span className="not-italic font-black uppercase tracking-widest text-[10px] text-primary/60 block mb-1">
              Order note
            </span>
            {order.note}
          </div>
